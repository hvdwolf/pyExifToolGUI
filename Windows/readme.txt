...
To build a Windows self contained executable that will run on any Windows system we need to have all the python scripts, the gui elements, the QT and pyside libraries and plugins, and the python dlls.
...
- run the following command (without the double quotes): 
...
- The last step is to manually copy the manual folder from the source folder
  into the dist\pyexiftoolgui folder that contains your created pyexiftool
  package.
